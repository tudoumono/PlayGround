"""
スマートExcel範囲比較プログラム（セルずれ対応版）
内容ベースのマッチングで質問事項の増減に対応
"""
...
推奨範囲: {suggested_address}"""
    ...
    def compare_ranges(self, range1_data: Dict, range2_data: Dict) -> List[Dict]:
        ...
        values1 = range1_data['values']  # 昨年度
        values2 = range2_data['values']  # 今年度
        ...
        for row_idx, row in enumerate(values1):
            for col_idx, value in enumerate(row):
                if value and str(value).strip():  # 空でないセルのみ
                    cells1.append({
                        'position': (row_idx, col_idx),
                        'value': str(value).strip(),
                        'address': self._get_cell_address(row_idx, col_idx)
                    })
        ...
        print(f"📊 昨年度: {len(cells1)}個の有効セル")
        print(f"📊 今年度: {len(cells2)}個の有効セル")
        
        # マッチング処理
        matched_pairs = []
        used_last_year_indices = set()
        unmatched_this_year = []
        
        print("\\n🔍 内容ベースマッチングを実行中...")
        
        for i, this_year_cell in enumerate(cells2):
            if i % 50 == 0 and i > 0:
                print(f"⏳ 進捗: {i}/{len(cells2)} ({i/len(cells2)*100:.1f}%)")
            ...
            # 昨年度の全セルから最適マッチを探す
            for j, last_year_cell in enumerate(cells1):
                if j in used_last_year_indices:
                    continue
                
                similarity = self.calculate_similarity(
                    this_year_cell['value'], 
                    last_year_cell['value']
                )
                
                if similarity > best_similarity:
                    best_similarity = similarity
                    best_match = last_year_cell
                    best_index = j
            
            # 閾値以上の場合はマッチとして記録
            if best_match and best_similarity >= self.match_threshold:
                matched_pairs.append({
                    'this_year_pos': this_year_cell['address'],
                    'this_year_value': this_year_cell['value'],
                    'last_year_pos': best_match['address'],
                    'last_year_value': best_match['value'],
                    'similarity': best_similarity,
                    'match_type': 'content_based'
                })
                used_last_year_indices.add(best_index)
            else:
                # マッチしなかった今年度セル
                unmatched_this_year.append({
                    'position': this_year_cell['address'],
                    'value': this_year_cell['value'],
                    'match_type': 'new_question'
                })
        
        # マッチしなかった昨年度セル
        unmatched_last_year = []
        for i, last_year_cell in enumerate(cells1):
            if i not in used_last_year_indices:
                unmatched_last_year.append({
                    'position': last_year_cell['address'],
                    'value': last_year_cell['value'],
                    'match_type': 'removed_question'
                })
        
        print(f"\\n📈 マッチング結果:")
        print(f"  ✅ マッチした質問: {len(matched_pairs)}ペア")
        print(f"  🆕 新規質問: {len(unmatched_this_year)}個")
        print(f"  🗑️ 削除された質問: {len(unmatched_last_year)}個")
        ...
        for match in matched_pairs:
            # 従来形式のstatusを維持
            if match['similarity'] == 100.0:
                status = "完全一致"
            elif match['similarity'] >= 80:
                status = "高類似"
            elif match['similarity'] >= 50:
                status = "中類似"
            else:
                status = "低類似/相違"
                
            all_results.append({
                'position': f"{match['this_year_pos']}⇔{match['last_year_pos']}",
                'value1': match['last_year_value'],
                'value2': match['this_year_value'],
        ...
        # 新規質問
        for new_q in unmatched_this_year:
            all_results.append({
                'position': f"{new_q['position']}(新規)",
                'value1': '',
                'value2': new_q['value'],
                'similarity': 0.0,
                'status': '新規質問',
        ...
        # 削除された質問
        for removed_q in unmatched_last_year:
            all_results.append({
                'position': f"{removed_q['position']}(削除)",
                'value1': removed_q['value'],
                'value2': '',
                'similarity': 0.0,
                'status': '削除された質問',
        ...
        print(f"⏱️ スマート比較完了: {elapsed_time:.2f}秒")
    ...
    def display_results(self, results: List[Dict], range1_data: Dict, range2_data: Dict):
        ...
        # ファイル情報を workbook_data から取得
        file1_info = self.workbook_data.get('file1', {})
        file2_info = self.workbook_data.get('file2', {})
        
        print(f"📄 昨年度ファイル: {file1_info.get('name', '不明')}")
        print(f"📍 昨年度範囲: {file1_info.get('range_address', range1_data.get('address', '不明'))}")
        print(f"📄 今年度ファイル: {file2_info.get('name', '不明')}")
        print(f"📍 今年度範囲: {file2_info.get('range_address', range2_data.get('address', '不明'))}")
        print()
        
        # 統計情報（スマート比較用）
        matched = [r for r in results if r.get('match_status') == 'matched']
        new_questions = [r for r in results if r.get('match_status') == 'new']
        removed_questions = [r for r in results if r.get('match_status') == 'removed']
        
        # マッチした質問の類似度別統計
        perfect_match = len([r for r in matched if r['similarity'] == 100.0])
        high_similarity = len([r for r in matched if 80 <= r['similarity'] < 100])
        medium_similarity = len([r for r in matched if 50 <= r['similarity'] < 80])
        low_similarity = len([r for r in matched if 30 <= r['similarity'] < 50])
        ...
        stats = {
            ...
            'removed_questions': len(removed_questions)
        }
        
        print("📈 スマート比較統計:")
        print(f"  📝 マッチした質問: {len(matched)}ペア")
        print(f"    ✅ 完全一致: {perfect_match}件")
        print(f"    🟡 高類似度(80-99%): {high_similarity}件")
        print(f"    🔵 中類似度(50-79%): {medium_similarity}件")
        print(f"    🟠 低類似度(30-49%): {low_similarity}件")
        print(f"  🆕 新規質問: {len(new_questions)}件")
        print(f"  🗑️ 削除された質問: {len(removed_questions)}件")
        print(f"  📊 合計: {len(results)}件の比較結果")
        print()
        
        # 詳細結果表示（主な変更点のみ）
        significant_changes = [r for r in results if r['similarity'] < 100.0 or r.get('match_status') in ['new', 'removed']]
        if significant_changes:
            print(f"🔍 主な変更点 ({len(significant_changes)}件):")
            print("-" * 80)
            for result in significant_changes[:20]:  # 最初の20件のみ表示
                match_status = result.get('match_status', 'matched')
                if match_status == 'matched':
                    print(f"📍 {result['position']} | {result['status']} ({result['similarity']:.1f}%)")
                    print(f"  昨年度: '{result['value1'][:50]}{'...' if len(result['value1']) > 50 else ''}'")
                    print(f"  今年度: '{result['value2'][:50]}{'...' if len(result['value2']) > 50 else ''}'")
                elif match_status == 'new':
                    print(f"🆕 {result['position']} | 新規質問")
                    print(f"  今年度: '{result['value2'][:50]}{'...' if len(result['value2']) > 50 else ''}'")
                elif match_status == 'removed':
                    print(f"🗑️ {result['position']} | 削除された質問")
                    print(f"  昨年度: '{result['value1'][:50]}{'...' if len(result['value1']) > 50 else ''}'")
                print()
                
            if len(significant_changes) > 20:
                print(f"... 他 {len(significant_changes) - 20} 件の変更があります")
        else:
    ...
    def _generate_html_report_content(self, results: List[Dict], range1_data: Dict, range2_data: Dict, stats: Dict) -> str:
        ...
        perfect_percent = (perfect_count / matched_pairs * 100) if matched_pairs > 0 else 0
        high_count = stats['high_similarity']
        high_percent = (high_count / matched_pairs * 100) if matched_pairs > 0 else 0
        medium_count = stats['medium_similarity']
        medium_percent = (medium_count / matched_pairs * 100) if matched_pairs > 0 else 0
        low_count = stats['low_similarity']
        low_percent = (low_count / matched_pairs * 100) if matched_pairs > 0 else 0
        new_count = stats['new_questions']
        removed_count = stats['removed_questions']
        ...
        # フィルター関連のスタイル
        html_parts.append('        .filter-controls { background: #f8fafc; padding: 20px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #e2e8f0; }')
        html_parts.append('        .search-box { margin-bottom: 15px; }')
        html_parts.append('        .search-box input { width: 100%; padding: 10px; border: 2px solid #e2e8f0; border-radius: 6px; font-size: 14px; }')
        html_parts.append('        .search-box input:focus { border-color: #4f46e5; outline: none; box-shadow: 0 0 0 3px rgba(79, 70, 229, 0.1); }')
        html_parts.append('        .filter-buttons { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 15px; }')
        html_parts.append('        .filter-btn { padding: 8px 16px; border: 2px solid #e2e8f0; background: white; border-radius: 20px; cursor: pointer; font-size: 13px; font-weight: 500; transition: all 0.2s; }')
        html_parts.append('        .filter-btn:hover { border-color: #4f46e5; background: #f0f4ff; }')
        html_parts.append('        .filter-btn.active { background: #4f46e5; color: white; border-color: #4f46e5; }')
        html_parts.append('        .filter-info { color: #6b7280; font-size: 14px; font-weight: 500; }')
        
        # バッジスタイル
        html_parts.append('        .badge { display: inline-block; padding: 4px 8px; border-radius: 12px; font-size: 11px; font-weight: bold; }')
        html_parts.append('        .badge-perfect { background: #dcfce7; color: #166534; }')
        html_parts.append('        .badge-high { background: #fef3c7; color: #92400e; }')
        html_parts.append('        .badge-medium { background: #dbeafe; color: #1e40af; }')
        html_parts.append('        .badge-low { background: #fee2e2; color: #dc2626; }')
        html_parts.append('        .badge-new { background: #dcfce7; color: #166534; }')
        html_parts.append('        .badge-removed { background: #fecaca; color: #dc2626; }')
        
        # 行スタイル（追加）
        html_parts.append('        .similarity-perfect { background: #f0fdf4 !important; }')
        html_parts.append('        .status-new { background: #f0fdf4 !important; }')
        html_parts.append('        .status-removed { background: #fef2f2 !important; }')
        ...
        html_parts.append('                        <h3>📄 昨年度ファイル</h3>')
        html_parts.append(f'                        <p><strong>ファイル名:</strong> {file1_info.get("name", "不明")}</p>')
        html_parts.append(f'                        <p><strong>フルパス:</strong> {file1_info.get("path", "不明")}</p>')
        html_parts.append(f'                        <p><strong>範囲:</strong> {file1_info.get("range_address", range1_data.get("address", "不明"))}</p>')
        ...
        html_parts.append('                    </div>')
        html_parts.append('                    <div class="file-card">')
        html_parts.append('                        <h3>📄 今年度ファイル</h3>')
        html_parts.append(f'                        <p><strong>ファイル名:</strong> {file2_info.get("name", "不明")}</p>')
        html_parts.append(f'                        <p><strong>フルパス:</strong> {file2_info.get("path", "不明")}</p>')
        html_parts.append(f'                        <p><strong>範囲:</strong> {file2_info.get("range_address", range2_data.get("address", "不明"))}</p>')
        ...
        html_parts.append(f'                        <div class="stat-number">{new_count:,}</div>')
        html_parts.append('                        <div>新規質問</div>')
        html_parts.append('                    </div>')
        html_parts.append('                    <div class="stat-card removed">')
        html_parts.append(f'                        <div class="stat-number">{removed_count:,}</div>')
        html_parts.append('                        <div>削除された質問</div>')
        html_parts.append('                    </div>')
        html_parts.append('                </div>')
        html_parts.append(f'                <p style="text-align: center; margin-top: 20px;"><strong>総比較結果: {total_results:,}件 (マッチ: {matched_pairs}件, 新規: {new_count}件, 削除: {removed_count}件)</strong></p>')
        html_parts.append('            </div>')
        
        # 結果詳細（フィルター機能付き）
        html_parts.append('            <div class="section">')
        html_parts.append(f'                <h2>🔍 比較結果詳細 ({len(results)}件)</h2>')
        
        # フィルターコントロール
        html_parts.append('                <div class="filter-controls">')
        html_parts.append('                    <div class="search-box">')
        html_parts.append('                        <input type="text" id="searchInput" placeholder="🔍 内容で検索..." onkeyup="filterTable()">')
        html_parts.append('                    </div>')
        html_parts.append('                    <div class="filter-buttons">')
        html_parts.append('                        <button class="filter-btn active" onclick="filterByStatus(\'all\')">すべて</button>')
        html_parts.append('                        <button class="filter-btn" onclick="filterByStatus(\'matched\')">マッチ</button>')
        html_parts.append('                        <button class="filter-btn" onclick="filterByStatus(\'new\')">新規</button>')
        html_parts.append('                        <button class="filter-btn" onclick="filterByStatus(\'removed\')">削除</button>')
        html_parts.append('                        <button class="filter-btn" onclick="filterByStatus(\'high\')">高類似度</button>')
        html_parts.append('                        <button class="filter-btn" onclick="filterByStatus(\'different\')">相違のみ</button>')
        html_parts.append('                    </div>')
        html_parts.append('                    <div class="filter-info">')
        html_parts.append('                        <span id="filterInfo">全 ' + str(len(results)) + ' 件を表示中</span>')
        html_parts.append('                    </div>')
        html_parts.append('                </div>')
        
        if results:
            html_parts.append('                <div class="table-container">')
            html_parts.append('                    <table class="table" id="resultsTable">')
            html_parts.append('                        <thead>')
            html_parts.append('                            <tr>')
            html_parts.append('                                <th style="width: 50px;">#</th>')
            html_parts.append('                                <th style="width: 80px;">ステータス</th>')
            html_parts.append('                                <th style="width: 120px;">位置</th>')
            html_parts.append('                                <th style="width: 80px;">類似度</th>')
            html_parts.append('                                <th style="width: 35%;">昨年度内容</th>')
            html_parts.append('                                <th style="width: 35%;">今年度内容</th>')
            html_parts.append('                            </tr>')
            html_parts.append('                        </thead>')
            html_parts.append('                        <tbody>')
            
            # すべての結果を出力（制限なし）
            print(f"📝 HTMLレポート: {len(results)}件の結果を全て出力中...")
            ...
            for i, result in enumerate(results, 1):
                ...
                if match_status == 'matched':
                    if result['similarity'] >= 80:
                        ...
                        status_text = '中類似'
                    elif result['similarity'] == 100:
                        row_class = 'similarity-perfect'
                    ...
                    row_class = 'status-removed'
                    badge_class = 'badge-removed'
                    status_text = '削除'
                
                # HTMLエスケープ処理
                value1 = str(result['value1']).replace('<', '&lt;').replace('>', '&gt;').replace('&', '&amp;')
                value2 = str(result['value2']).replace('<', '&lt;').replace('>', '&gt;').replace('&', '&amp;')
                
                # 長いテキストは省略表示（ツールチップで全文表示）
                display_value1 = value1[:80] + ('...' if len(value1) > 80 else '')
                display_value2 = value2[:80] + ('...' if len(value2) > 80 else '')
                
                html_parts.append(f'                        <tr class="{row_class}" data-status="{match_status}" data-similarity="{result["similarity"]}">')
                html_parts.append(f'                            <td>{i}</td>')
                html_parts.append(f'                            <td><span class="badge {badge_class}">{status_text}</span></td>')
                html_parts.append(f'                            <td>{result["position"]}</td>')
                html_parts.append(f'                            <td>{result["similarity"]:.1f}%</td>')
                ...
                # 進捗表示（1000件ごと）
                if i % 1000 == 0:
                    print(f"  ... {i}件 / {len(results)}件 処理完了")
            
            print(f"✅ HTMLレポート: 全{len(results)}件の結果出力完了")
        ...
        html_parts.append('    let currentFilter = "all";')
        html_parts.append('    ')
        html_parts.append('    function filterByStatus(status) {')
        html_parts.append('        currentFilter = status;')
        html_parts.append('        ')
        html_parts.append('        // ボタンのアクティブ状態を更新')
        html_parts.append('        document.querySelectorAll(".filter-btn").forEach(btn => btn.classList.remove("active"));')
        html_parts.append('        event.target.classList.add("active");')
        html_parts.append('        ')
        html_parts.append('        applyFilters();')
        html_parts.append('    }')
        html_parts.append('    ')
        html_parts.append('    function filterTable() {')
        html_parts.append('        applyFilters();')
        html_parts.append('    }')
        html_parts.append('    ')
        html_parts.append('    function applyFilters() {')
        html_parts.append('        const searchText = document.getElementById("searchInput").value.toLowerCase();')
        html_parts.append('        const table = document.getElementById("resultsTable");')
        html_parts.append('        const rows = table.querySelectorAll("tbody tr");')
        html_parts.append('        let visibleCount = 0;')
        html_parts.append('        ')
        html_parts.append('        rows.forEach((row, index) => {')
        html_parts.append('            let showRow = true;')
        html_parts.append('            ')
        html_parts.append('            // ステータスフィルター')
        html_parts.append('            if (currentFilter !== "all") {')
        html_parts.append('                const status = row.dataset.status;')
        html_parts.append('                const similarity = parseFloat(row.dataset.similarity);')
        html_parts.append('                ')
        html_parts.append('                switch(currentFilter) {')
        html_parts.append('                    case "matched":')
        html_parts.append('                        showRow = status === "matched";')
        html_parts.append('                        break;')
        html_parts.append('                    case "new":')
        html_parts.append('                        showRow = status === "new";')
        html_parts.append('                        break;')
        html_parts.append('                    case "removed":')
        html_parts.append('                        showRow = status === "removed";')
        html_parts.append('                        break;')
        html_parts.append('                    case "high":')
        html_parts.append('                        showRow = status === "matched" && similarity >= 80;')
        html_parts.append('                        break;')
        html_parts.append('                    case "different":')
        html_parts.append('                        showRow = similarity < 100 || status !== "matched";')
        html_parts.append('                        break;')
        html_parts.append('                }')
        html_parts.append('            }')
        html_parts.append('            ')
        html_parts.append('            // 検索フィルター')
        html_parts.append('            if (showRow && searchText) {')
        html_parts.append('                const cells = row.querySelectorAll("td");')
        html_parts.append('                const rowText = Array.from(cells).map(cell => cell.textContent.toLowerCase()).join(" ");')
        html_parts.append('                showRow = rowText.includes(searchText);')
        html_parts.append('            }')
        html_parts.append('            ')
        html_parts.append('            // 行の表示/非表示')
        html_parts.append('            row.style.display = showRow ? "" : "none";')
        html_parts.append('            if (showRow) visibleCount++;')
        html_parts.append('        });')
        html_parts.append('        ')
        html_parts.append('        // フィルター情報を更新')
        html_parts.append('        const filterInfo = document.getElementById("filterInfo");')
        html_parts.append('        const totalCount = rows.length;')
        html_parts.append('        ')
        html_parts.append('        if (currentFilter === "all" && !searchText) {')
        html_parts.append('            filterInfo.textContent = `全 ${totalCount} 件を表示中`;')
        html_parts.append('        } else {')
        html_parts.append('            filterInfo.textContent = `${visibleCount} / ${totalCount} 件を表示中`;')
        html_parts.append('        }')
        html_parts.append('    }')
        html_parts.append('    ')
        html_parts.append('    // 初期化')
        html_parts.append('    document.addEventListener("DOMContentLoaded", function() {')
        html_parts.append('        console.log("フィルター機能が初期化されました");')
        html_parts.append('    });')
        html_parts.append('    </script>')
    ...
    def _create_summary_sheet_safe(self, report_wb, range1_data: Dict, range2_data: Dict, stats: Dict):
        ...
        report_data.append(("A16", f"総比較結果: {stats.get('total_results', 0)}"))
...
def main():
    """メイン関数（繰り返し実行対応）"""
    print("🚀 スマートExcel範囲比較プログラム（セルずれ対応版）を起動します...")
    print("="*60)
    
    comparator = SmartRangeComparator()
